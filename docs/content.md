...
- **Pins & paths** — a small tool rail: drop labeled pins, click-click-click a
  path and double-click (or ✓) to finish. Click any annotation to relabel or
  delete it. Everything carries its author's color.
...
- **Base map** — vector tiles from [OpenFreeMap](https://openfreemap.org) by
  ...
  the feature leans on). Point `MAP_STYLE_URL` at your own MapLibre style to
  stay fully self-contained, or override it just for yourself in
  **Settings → Maps**. The MapLibre bundle is lazy-loaded only when a map
  board opens.
...
| `POST /api/boards/{id}/features` | Map: add an annotation `{kind: pin·path, coords, label?, color?}` — live viewers see it instantly |
