...
- 🗺️ **Map boards** *(MapLibre + Yjs)* — a shared map the room annotates together: **pins and paths** sync live (and are scriptable over the API), plus **live location sharing on your terms** — for 15 minutes, an hour, or until you stop; positions are ephemeral (never stored) and vanish when you stop. Default tiles from OpenFreeMap, swappable via `MAP_STYLE_URL` or per-user in Settings.
