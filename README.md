...
- 🗺️ **Map boards** *(MapLibre + Yjs)* — a shared map the room annotates together: **pins with popped-up labels, paths with visible nodes, freehand drawings, story-style text, and images you drag around like a canvas** — all synced live (and scriptable over the API), organized in a **layers menu** (hide/show, zoom-to, delete). Plus **live location sharing on your terms** — 15 minutes, an hour, or until you stop; positions are ephemeral (never stored). Base style switchable on the map, via `MAP_STYLE_URL`, or per-user in Settings.
