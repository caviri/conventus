        ...
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          <section className="card p-4">
            ...
            <label className="mb-1 mt-3 block text-xs font-medium text-[var(--c-muted)]">
              Avatar (generated by default — emoji, image URL, or upload)
            </label>
            <div className="flex items-center gap-2">
              ...
              <input
                className="input"
                value={avatar}
                placeholder="leave blank for a generated icon, or 🦊 / https://…"
                onChange={(e) => setAvatarVal(e.target.value)}
                onKeyDown={(e) => e.key === "Enter" && saveAvatar(avatar)}
              />
              <button
                className="btn shrink-0"
                title="Generate a new icon"
                onClick={() => {
                  const next = randomProcAvatar();
                  setAvatarVal(next);
                  saveAvatar(next);
                }}
              >
              ...
              <button className="btn shrink-0" title="Upload an image" onClick={() => avatarFile.current?.click()}>
                <Upload size={15} />
              </button>
